...
**Renderer:** Kroki API (`https://kroki.io`) — just `curl`, no Java needed. Fallback chain: `plantumlgen.pythonanywhere.com` → local `plantuml.jar` (Java 11+).
**Output:** PNG, SVG
...
| **Batch** | to render all `.puml` files from `uml/plantuml/` to PNG, or convert use case `.md` files to PlantUML | [Batch Render](#batch-render-mode) |
...
Render file `.puml` dari `uml/plantuml/` ke `uml/plantuml/out/`.

1. List semua file `.puml` di `uml/plantuml/` (skip `out/`)
2. **Tanya user** file mana yang dirender
3. Pastikan folder `uml/plantuml/out/` ada
4. Render via Kroki API (dengan fallback chain jika gagal):
   ```bash
   # Coba Kroki
   curl -s -o uml/plantuml/out/<nama>.png --data-binary "@uml/plantuml/<nama>.puml" https://kroki.io/plantuml/png
   
   # Jika < 1000 bytes → fallback ke PlantUMLGen
   if [ $(wc -c < uml/plantuml/out/<nama>.png) -lt 1000 ]; then
     ENCODED=$(python3 -c "import urllib.parse; print(urllib.parse.quote(open('uml/plantuml/<nama>.puml').read()))")
     curl -s -o uml/plantuml/out/<nama>.png "https://plantumlgen.pythonanywhere.com/generate_uml?uml_text=$ENCODED"
   fi
   
   # Jika masih < 1000 bytes → fallback ke local JAR
   if [ $(wc -c < uml/plantuml/out/<nama>.png) -lt 1000 ]; then
     java -jar bin/plantuml.jar -tpng uml/plantuml/<nama>.puml -o uml/plantuml/out/
   fi
   ```
5. Validasi: HTTP 200, file PNG valid
6. Laporkan hasil

### B. Konversi Use Case `.md` → `.puml` → PNG

Cari dan konversi file `.md` yang mengandung "use case" di luar folder `uml/plantuml/`.

1. **Cari** file `*USE_CASE*.md` atau `*.md` yang mengandung kata "use case" di current working directory (skip `uml/` folder)
2. **Tanya user** file mana yang dikonversi
3. **Konversi ke PlantUML** dengan aturan:
   - Aktor eksternal di kiri & kanan (`-down->` / `-up->`)
   - `rectangle "System" as boundary { }` — boundary tengah
   - `left to right direction` + `skinparam packageStyle rectangle`
   - Use case internal (scheduler, worker) tanpa panah aktor
   - "Sistem" BUKAN aktor
4. Simpan `.puml` ke `uml/plantuml/<nama>.puml`
5. Render via Kroki → simpan PNG ke `uml/plantuml/out/<nama>.png`
