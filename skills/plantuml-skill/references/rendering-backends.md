...
| **PlantUMLGen** (fallback) | `https://plantumlgen.pythonanywhere.com/generate_uml?uml_text=<encoded>` | just `curl` | **Yes** — sent as URL-encoded query param |
...
## Fallback: Kroki → PlantUMLGen → Local JAR

When Kroki returns HTTP 400 or a file under 1000 bytes, automatically fall back
through the chain:

**1. PlantUMLGen** (`plantumlgen.pythonanywhere.com`) — GET-based with URL-encoded `uml_text`:

```bash
ENCODED=$(python3 -c "import urllib.parse; print(urllib.parse.quote(open('diagram.puml').read()))")
curl -s -o output.png "https://plantumlgen.pythonanywhere.com/generate_uml?uml_text=$ENCODED"
```

**2. Local plantuml.jar** (if Java available) — zero network, fastest for complex diagrams:

```bash
java -jar bin/plantuml.jar -tpng diagram.puml -o output/
```

MCP server available at `bin/plantuml-mcp.py` with `render_plantuml` tool.
Requires Java 11+ on PATH.

**Fallback order:** Kroki → PlantUMLGen → Local JAR → report failure.
Do not skip intermediate steps — each is tried automatically.
